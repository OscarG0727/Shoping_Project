...
for (let i = 0; i < botons.length; i++) {
  ...
  botons[i].addEventListener("click", () => {
    ...
    const nameProd = document.getElementById(
      `prodName-${i}`
    ); /*se localiza la variable mediante id y se guarda en una nueva variable */
    const divProdName = document.getElementById(
      nameProd.dataset.value
    ); /*se obtiene el valor de data a traves de la variable creada anteriormente */
    if (divProdName) {
      /*si la variable se encuentra */
      const spanCounter =
        divProdName.querySelector(
          ".counter"
        ); /*Seleccionamos con la clase .counter y guardamos el valor en una nueva variable */
      if (spanCounter) {
        ...
        const quantityResult = divProdName.querySelector(".quantity");
        if (quantityResult) {
          const quantityValue =
            (counterValue + 1) * Number(divPriceItem.dataset.value);
      ...
      const pricesItem = document.getElementById(
        `priceItem-${i}`
      ); /*en una variable guardamos el getElement de priceItem */
      const divPriceItem = document.getElementById(
        pricesItem.dataset.value
      ); /*en otra variable guardaremos la variable anterior junto con el metodo dataset */
